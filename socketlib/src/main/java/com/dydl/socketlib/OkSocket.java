...
public class OkSocket {
    ...
    // 发送请求字符码Bean(传bean格式)//代码暂时不维护了
    public static <T> void sendMsgBean(final OkBase bean,
                                       final int port,
                                       Class<? extends OkResponse> clazz,
                                       final OkCallBack listener) {
        OkStr2Helper.<T>sendCreateData(bean, port, clazz, listener);

    }

    // 发送请求字符码String(传bean格式)//代码暂时不维护了
    public static void sendMsgString(final OkBase bean, final int port,
                                     final OkCallBack<String> listener) {
        OkStr2Helper.sendCreateDataString(bean, port, listener);
    ...
    public static void sendMsgXml(final OkXmlBean bean, final int port, final OkCallBack listener) {
        listener.onStart();
        OkXmlHelper.sendCreateDataString(bean, port, listener);
    }
